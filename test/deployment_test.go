...
var repoName string = "terraform-aws-vault-ent-starter"
...
func TestClusterDeployment(t *testing.T) {
	...
	licenseLocation := filepath.Join(cwdPath, "vault.hclic")
	if _, err := os.Stat(licenseLocation); errors.Is(err, os.ErrNotExist) {
		if os.Getenv("TEST_VAULT_ENT_LICENSE") != "" {
			err := os.WriteFile(licenseLocation, []byte(os.Getenv("TEST_VAULT_ENT_LICENSE")), 0644)
			if err != nil {
				logger.Log(t, err)
				t.FailNow()
			}
		} else {
			logger.Log(t, "Vault license needs to be in place, or in the TEST_VAULT_ENT_LICENSE environment variable")
			t.FailNow()
		}
	}
